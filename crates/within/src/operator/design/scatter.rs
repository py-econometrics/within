...
use crate::domain::{FactorRows, PreparedDesign, TermMeta};
    ...
    for (q, t) in design.terms.iter().enumerate() {
        ...
        match &*t.columns {
            ...
            [Loading::Constant, Loading::Covariate(c0), Loading::Covariate(c1)] => {
                ...
                scatter_term::<3>(block, t, rows, parallel, scratch, |i| {
                    let b = base(i);
                    [b, z0[i] * b, z1[i] * b]
                })
            }
            ...
            columns => {
                for (c, loading) in columns.iter().enumerate() {
                    ...
                    match loading {
                        Loading::Constant => {
                            scatter_term::<1>(slot, t, rows, parallel, scratch, |i| [base(i)]);
                        }
                        Loading::Covariate(k) => {
                            let z = prepared.loading_column(*k as usize);
                            scatter_term::<1>(slot, t, rows, parallel, scratch, move |i| {
                                [z[i] * base(i)]
                            });
                        }
                    }
...
fn scatter_term<const C: usize>(
    ...
    match ScatterStrategy::pick(parallel, C * n_levels, rows.sorted()) {
