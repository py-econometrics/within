...
fn solve_and_assemble<A: Operator + ?Sized>(
    op: &A,
    x: Vec<f64>,
    converged: bool,
    iterations: usize,
...
impl<'a, S: ObservationStore> AssemblyMode<'a> for ExplicitAssembly<'a, S> {
    ...
    fn prepare(self) -> Self::Prepared {
        let (domains, blocks) = build_domains_and_gramian_blocks(self.design);
...
impl<'a, S: ObservationStore> AssemblyMode<'a> for ImplicitAssembly<'a, S> {
    ...
    fn prepare(self) -> Self::Prepared {
        ImplicitPrepared {
            design: self.design,
            domains: build_local_domains(self.design),
        }
    }
...
impl<'a> NormalEquationSolver<'a> {
    ...
    fn solve(&self, rhs: &[f64]) -> WithinResult<MethodSolve> {
