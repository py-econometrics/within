...
/// Square sparse matrix in CSR format. Block elimination produces the CSR invariants by construction, so [`CsrMatrix::new`] is infallible and they are guarded only in debug builds.
#[derive(Clone)]
...
impl CsrMatrix {
    /// Create a `CsrMatrix` from raw CSR components, trusted in release and `assert!`-checked in debug.
