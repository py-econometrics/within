...
const PRECOND_BYTES: &[u8] = include_bytes!("fixtures/preconditioner_v4.postcard");
const PRE_BUMP_BYTES: &[u8] = include_bytes!("fixtures/preconditioner_v3.postcard");

fn fixture_problem() -> (Vec<u32>, Vec<u32>, Vec<f64>, Vec<f64>) {
    // Slope term against a three-level factor: the (f-slope, g) pair is
    // frustrated (negative 4-cycle), so the payload pins Cover coordinates
    // on the wire alongside the plain (f-int, g) pair's Canonical ones.
...
/// `crates/within/tests/fixtures/preconditioner_v4.postcard`. Intended for
...
fn regenerate_wire_format_fixture() {
    ...
    path.push("tests/fixtures/preconditioner_v4.postcard");
