use within::{LocalSolverConfig, OperatorRepr, SolverMethod, SolverParams, WeightedDesign};
...
fn test_solve_row_major_matches_factor_major() {
    ...
    let r_fm = common::solve_response(&dm_fm, &y, &params).expect("fm solve");
    ...
    let r_rm = common::solve_response(&dm_rm, &y, &params).expect("rm solve");
...
fn test_solve_compressed_matches_factor_major() {
    ...
    let r_cs = common::solve_response(&dm_cs, &y, &params).expect("compressed solve");
