...
//!
//! - [`SubdomainCore`] — restriction `Rᵢ` and partition-of-unity weighting `D̃ᵢ`
//!   for one subdomain; paired with a [`LocalSolver`](crate::local_solve::LocalSolver)
//!   to form a Schwarz term `Rᵢᵀ D̃ᵢ Aᵢ⁻¹ D̃ᵢ Rᵢ`.
//! - [`PartitionWeights`] — per-DOF weights, uniform constant or explicit vector.

use std::sync::atomic::{AtomicU64, Ordering};

use crate::error::BuildError;

/// Partition-of-unity weights for a subdomain; most are uniformly 1.0, so that case avoids allocating a `Vec<f64>`.
...
/// A domain-agnostic subdomain core implementing `Rᵢ` and `D̃ᵢ`: sorted global DOF indices plus partition-of-unity weights, so [`restrict_weighted`](Self::restrict_weighted) is `D̃ᵢ Rᵢ r` and [`prolongate_weighted_add`](Self::prolongate_weighted_add) is `out += Rᵢᵀ D̃ᵢ z`.
#[derive(Clone)]
