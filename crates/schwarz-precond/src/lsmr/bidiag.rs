...
/// Below this count the per-iteration vector kernels run sequentially, since rayon wake/steal overhead would dominate.
pub(super) const LSMR_PAR_THRESHOLD: usize = 10_000;
/// Per-worker chunk size for the parallel vector kernels: large enough to clear rayon dispatch, small enough to stay L1-resident.
pub(super) const LSMR_UPDATE_CHUNK: usize = 4096;

/// Fused `y = x + scale · y` returning `‖y_new‖²`; parallel above the threshold, each chunk accumulating its own partial to avoid cross-thread reduction traffic.
#[inline]
...
/// `α = √⟨v, p̃⟩`. A near-breakdown negative `vp` within the relative floor `√ε·‖v‖‖p̃‖` clamps to `α = 0`; a genuinely indefinite preconditioner raises.
...
/// Windowed ring of recent basis vectors for windowed modified Gram-Schmidt, holding `cap` slots of `L` parallel length-`n` lanes. Ring mechanics live here; the inner product and MGS sweep come from the `L`-specialized impls below. The disabled state is `Option<WindowRing<L>> = None`, so this is only ever built with positive capacity.
struct WindowRing<const L: usize> {
    /// `L` flat buffers, each `cap · n` long; lane `l`, slot `s` is the window `[s*n .. s*n + n]` of `lanes[l]`.
    lanes: [Vec<f64>; L],
    n: usize,
    next: usize,
    count: usize,
...
impl<const L: usize> WindowRing<L> {
    /// `None` when no reorthogonalization is requested. Capped at `min(m, n)`, the most linearly independent basis vectors the bidiagonalization can produce before α or β hits zero.
...
impl WindowRing<1> {
    /// Modified Gram-Schmidt sweep over stored slots oldest-first, subtracting the projection of `y` onto each.
...
/// M-weighted windowed MGS over the paired `(v, p̃ = M v)` lanes: `v` is M-orthogonal rather than Euclidean, so the coefficient is `⟨v_new, p̃_j⟩` and subtracting it from both lanes in lockstep preserves the invariant.
impl WindowRing<2> {
    /// M-weighted MGS over stored `(v_j, p̃_j)` pairs; the coefficient `c = ⟨v, p̃_j⟩` is subtracted from both `v` and `p̃`, keeping `p̃ = M v` consistent.
    ...
    /// Copy the normalized `v` and `p_tilde · inv_alpha` (= `M · v_norm`) into the paired next slots, advancing the ring.
...
/// A bidiagonalization stream feeding LSMR with `(α, β)` pairs and the matching normalized basis vector `v_k`.
...
impl<A: Operator + ?Sized> Bidiagonalization for GolubKahan<'_, A> {
    fn step(&mut self) -> Result<BidiagStep, SolveError> {
        ...
        if beta == 0.0 {
            // Lucky breakdown: zero `v` so `solution.update` contributes nothing, and α = 0 keeps the rotation well-defined.
    ...
    for ModifiedGolubKahan<'_, A, M>
{
    fn step(&mut self) -> Result<BidiagStep, SolveError> {
        ...
        let beta = beta_sq.sqrt();
        if beta == 0.0 {
            // Lucky breakdown: zero `v` and the paired `p̃` so `solution.update` contributes nothing this step.
            self.bufs.v.fill(0.0);
...
/// Standard Golub-Kahan bidiagonalization: no preconditioner, no `p̃` buffer, two normalizations per step.
...
impl<'a, A: Operator + ?Sized> GolubKahan<'a, A> {
    ...
    pub(super) fn init(
        ...
        // Max-scaled norm: `par_dot(b, b).sqrt()` overflows to ∞ for large-magnitude `b`, zeroing u₁ into a silent `x = 0`.
...
struct ModifiedGolubKahanBuffers {
    /// `u` in observation space, left unnormalized between steps so `‖u‖ = β_{k+1}`; the next adjoint matvec yields `β · Aᵀ u_norm` and the `β/α_prev` coefficient on `p_tilde` cancels the unnormalization.
    u: Vec<f64>,
...
/// Modified Golub-Kahan bidiagonalization with `M ≈ AᵀA`, storing `p̃` scaled by `α` between steps so each iteration costs one `M⁻¹` application.
...
impl<'a, A: Operator + ?Sized, M: Operator + ?Sized> ModifiedGolubKahan<'a, A, M> {
    ...
    /// Phase 2 of [`step`](Bidiagonalization::step): fold `β · Aᵀ u_norm` into the `p̃` recurrence, scaling by `β / α_k` so the stored `α_k` cancels and `p̃ = α_new · M v_new` again. Requires `α_k > 0`, which the outer loop guarantees.
    ...
    /// Phase 3 of [`step`](Bidiagonalization::step): recover `ṽ = M⁻¹ p̃`, run M-weighted MGS mutating `v` and `p_tilde` in lockstep to hold `p̃ = M v`, then normalize and push the pair; returns the corrected `α_{k+1} = √⟨ṽ, p̃⟩`.
