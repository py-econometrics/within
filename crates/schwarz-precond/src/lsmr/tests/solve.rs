...
fn test_lsmr_large_magnitude_rhs_not_silently_zero() {
    ...
    let result =
        lsmr(&IdentityOp { n: 3 }, &b, 1e-10, 100, Default::default()).expect("lsmr solve");
...
fn test_mlsmr_unpreconditioned() {
    ...
    let result = lsmr(&OverdeterminedOp, &b, 1e-10, 100, Default::default()).expect("lsmr solve");
...
fn test_mlsmr_underdetermined_system() {
    ...
    let b = vec![1.0, 2.0];
    let result = lsmr(&UnderOp, &b, 1e-12, 100, Default::default()).expect("underdetermined solve");
...
fn test_mlsmr_rank_deficient_system() {
    ...
    let b = vec![3.0, 6.0];
    let result =
        lsmr(&RankDeficientOp, &b, 1e-12, 100, Default::default()).expect("rank-deficient solve");
    assert!(result.converged);
...
fn test_mlsmr_zero_column_and_zero_row() {
    let b = vec![2.0, 3.0];
    let result =
        lsmr(&ZeroSecondRow, &b, 1e-12, 100, Default::default()).expect("degenerate solve");
    assert!(result.converged);
...
fn test_mlsmr_maxiter_exhaustion() {
    let b = vec![1.0, 2.0, 3.0, 3.0];
    let result = lsmr(&OverdeterminedOp, &b, 1e-15, 1, Default::default()).expect("lsmr solve");
    assert!(
        !result.converged,
...
fn test_mlsmr_none_matches_identity_precond() {
    ...
    let none_result =
        lsmr(&OverdeterminedOp, &b, 1e-12, 100, Default::default()).expect("lsmr solve");
    let id_result = mlsmr(
        &OverdeterminedOp,
        &b,
        &id,
...
fn test_mlsmr_none_matches_identity_precond_windowed() {
    ...
    let none_result = lsmr(&op, &b, 1e-12, 50, window(local)).expect("lsmr windowed solve");
...
fn test_mlsmr_local_reorth_zero_is_identity() {
    let b = vec![1.0, 2.0, 3.0, 3.0];
    let r1 = lsmr(&OverdeterminedOp, &b, 1e-10, 100, Default::default()).expect("unwindowed solve");
    let r2 = lsmr(&OverdeterminedOp, &b, 1e-10, 100, window(Some(0))).expect("zero-window solve");
...
fn test_mlsmr_local_reorth_unpreconditioned() {
    ...
    let r0 = lsmr(&op, &b, tol, maxiter, Default::default()).expect("no-reorth solve");
    let r10 = lsmr(&op, &b, tol, maxiter, window(Some(10))).expect("windowed solve");
...
fn test_mlsmr_local_reorth_window_boundary_sizes(#[case] window_size: usize) {
    ...
    let result =
        lsmr(&op, &b, 1e-9, 200, window(Some(window_size))).expect("lsmr boundary-window solve");
    assert!(result.converged);
    assert!(
