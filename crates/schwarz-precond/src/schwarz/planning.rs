...
pub enum ReductionStrategy {
    ...
    #[default]
    Auto,
    /// Each subdomain atomically scatters into a shared accumulator; O(n_dofs) shared plus O(P · max_scratch) thread-local.
    AtomicScatter,
    /// Each task accumulates into a private buffer, then a parallel chunk reduction combines them; O(P · n_dofs) for task buffers.
    ParallelReduction,
}
...
/// Build-time scheduling metrics and `Auto` resolution. Holds only metrics needing a walk over entries; sizes the preconditioner already knows are passed at call time.
#[derive(Debug, Clone, Copy)]
