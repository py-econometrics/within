...
impl<S: LocalSolver> AdditiveExecutor<S> {
    ...
    fn apply_atomic(
        ...
        // A pool-recovery failure must not mask a real `LocalSolveFailed`, so prefer the original error when it is one.
        ...
        // On apply error the swap-zero readout is skipped, leaving `accum` dirty, so `put` drops the whole buffer.
        apply_result?;
