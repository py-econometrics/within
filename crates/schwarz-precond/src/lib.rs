...
/// A linear operator `A: R^ncols -> R^nrows` with its adjoint, `Send + Sync` for Rayon. Both applies are fallible even for implementors that cannot fail, so callers get one uniform `?` path.
pub trait Operator: Send + Sync {
    /// Number of rows in the operator.
    fn nrows(&self) -> usize;
    /// Number of columns in the operator.
    fn ncols(&self) -> usize;
    /// Computes `y = A x`, erroring if the apply fails at runtime (e.g. a local subdomain solver diverges).
