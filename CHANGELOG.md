...
- `PreconditionerConfig::Adaptive` starts on the diagonal and escalates to additive Schwarz on a stalled contraction, building the Schwarz factorization only on escalation; `Solver::has_escalated()` reports whether the Schwarz map was built (#260). Before escalation `Solver::preconditioner()` returns the diagonal base carrying the ladder (`config()` is `Adaptive`), so a solver that reuses or deserializes it escalates too (#410).
