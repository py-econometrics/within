...
- LSMR certified a stop on an operator whose `‖A‖_F` estimate overflows to infinity: the normal-equation ratio divided to zero and accepted any residual. `A = diag(1.2e154, 1e154)` stopped after one iteration at a residual of 0.25 and now solves in two (#297).
- The LSMR true-residual audit measures `‖Aᵀr‖` outside the preconditioner's metric as well as inside it, referenced to `‖Aᵀb‖` and, for a warm start, re-based onto the original `b` like the metric leg. A preconditioner with a null direction reported that part of the residual as zero and certified a stop that had not solved along it (#297).
