...
- Designs firing that warning now get an exact sparse LDLᵀ solve over the warned terms, applied additively on top of the base preconditioner; low-mobility shared-covariate panels that exhausted a 3000-iteration budget converge in ~25 iterations. When the symbolic factor's fill exceeds its budget, a factored sparse approximate inverse over the same block takes its place (~1200 iterations to ~64 on a mid-scale mid-mobility panel). `within` gains a `faer` dependency for the factorization (#281).
