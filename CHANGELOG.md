...
- Designs firing that warning now get an exact sparse LDLᵀ solve over the warned terms, applied additively on top of the base preconditioner; low-mobility shared-covariate panels that exhausted a 3000-iteration budget converge in ~25 iterations. `within` gains a `faer` dependency for the factorization (#281).
