...
- The LSMR true-residual audit measures `‖Aᵀr‖` outside the preconditioner's metric as well as inside it, against `‖Aᵀb‖` captured at initialization. A preconditioner with a null direction reported that part of the residual as zero and certified a stop that had not solved along it (#297).
