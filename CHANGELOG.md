...
- **BREAKING:** The serialized `Preconditioner` wire format moved v12 → v18 (approx-chol 0.5, full construction config, build duration, `LocalSolverConfig::ridge`, the built map's own Schwarz description in place of the strategy enum, and a 256-bit design-layout signature); 0.3.0 bytes no longer decode.
- Varying-slope solver coordinates keep a level's coefficient columns adjacent; public coefficient order remains column-major, and design signatures encode the internal mapping (#271).
