...
- **BREAKING:** The serialized `Preconditioner` wire format changed with the `approx-chol` 0.4 → 0.5 bump (v12 → v13), retention of the complete construction config (v13 → v14), retention of its original build duration (v14 → v15), the new `LocalSolverConfig::ridge` field (v15 → v16), and the cross-term gauge constraint the preconditioner now carries (v16 → v17); 0.3.0 bytes no longer decode.
