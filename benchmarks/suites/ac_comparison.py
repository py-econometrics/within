...
def run_ac_comparison(opts: SuiteOptions) -> list[BenchmarkResult]:
    if opts.quick:
        ...
        problems = [
            ...
            ProblemSpec("AKM Power-Law", "akm_power_law", {"n_workers": 5000, "n_firms": 200, "n_years": 10}, opts.seed),
        ]

    configs = [
        # One-level additive
        SolverConfig("CG(1L, AC)", CG(tol=opts.tol, maxiter=opts.maxiter, preconditioner=OneLevelSchwarz(smoother=_ac(opts.seed)))),
        SolverConfig("CG(1L, AC2)", CG(tol=opts.tol, maxiter=opts.maxiter, preconditioner=OneLevelSchwarz(smoother=_ac2(opts.seed)))),
        # Multiplicative one-level (symmetric for CG)
        SolverConfig("CG(M1L, AC)", CG(tol=opts.tol, maxiter=opts.maxiter, preconditioner=MultiplicativeOneLevelSchwarz(smoother=_ac(opts.seed)))),
        SolverConfig("CG(M1L, AC2)", CG(tol=opts.tol, maxiter=opts.maxiter, preconditioner=MultiplicativeOneLevelSchwarz(smoother=_ac2(opts.seed)))),
    ]
    ...
    print_table(all_results, columns=["problem", "config", "setup_time", "solve_time", "iterations", "final_residual", "converged"])
    print("\n--- Iterations pivot ---")
