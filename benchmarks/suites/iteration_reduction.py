...
def run_iteration_reduction(opts: SuiteOptions) -> list[BenchmarkResult]:
    ...
    if opts.quick:
        problems = [
            ProblemSpec("2-FE 30x40", "uniform_kfe", {"n_levels_per_factor": [30, 40], "n_rows": 2000}, opts.seed),
            ProblemSpec("3-FE 30x40x25", "uniform_kfe", {"n_levels_per_factor": [30, 40, 25], "n_rows": 2000}, opts.seed),
        ]
    else:
        problems = [
            ProblemSpec("2-FE 30x40", "uniform_kfe", {"n_levels_per_factor": [30, 40], "n_rows": 2000}, opts.seed),
            ProblemSpec("2-FE 50x80", "uniform_kfe", {"n_levels_per_factor": [50, 80], "n_rows": 5000}, opts.seed),
            ProblemSpec("3-FE 30x40x25", "uniform_kfe", {"n_levels_per_factor": [30, 40, 25], "n_rows": 2000}, opts.seed),
            ProblemSpec("3-FE 50x80x30", "uniform_kfe", {"n_levels_per_factor": [50, 80, 30], "n_rows": 5000}, opts.seed),
        ]
    ...
    configs_cg = [
        ...
        SolverConfig("CG(Schwarz)", CG(tol=opts.tol, maxiter=maxiter, preconditioner=OneLevelSchwarz(smoother=ApproxCholConfig(seed=opts.seed)))),
        SolverConfig("GMRES(Mult-Schwarz)", GMRES(tol=opts.tol, maxiter=maxiter, preconditioner=MultiplicativeOneLevelSchwarz(smoother=ApproxCholConfig(seed=opts.seed)))),
        SolverConfig("CG(Mult-Schwarz)", CG(tol=opts.tol, maxiter=maxiter, preconditioner=MultiplicativeOneLevelSchwarz(smoother=ApproxCholConfig(seed=opts.seed)))),
    ]
    ...
    print_table(all_results, columns=["problem", "config", "iterations", "final_residual", "converged"])
    ...
    for prob in problems:
        ...
        r_none = next((r for r in all_results if r.problem == prob.name and r.config == label_none), None)
        r_prec = next((r for r in all_results if r.problem == prob.name and r.config == label_prec), None)
        ...
        print(f"{prob.name:<18} | {'CG':<10} | {i_none:>7} | {i_prec:>7} | {reduction:>9}")
